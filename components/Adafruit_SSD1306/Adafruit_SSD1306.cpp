...
//		Adafruit_GFX(w, h), spi(NULL), wire(twi ? twi : &Wire), buffer(NULL), mosiPin(
//				-1), clkPin(-1), dcPin(-1), csPin(-1), rstPin(rst_pin), wireClk(
//				clkDuring), restoreClk(clkAfter) {
//}

Adafruit_SSD1306::Adafruit_SSD1306(uint8_t w, uint8_t h, TwoWire *twi,
		int8_t rst_pin, uint32_t clkDuring, uint32_t clkAfter) :
		Adafruit_GFX(w, h), spi(NULL), wire(twi ? twi : &Wire), buffer(NULL), mosiPin(
				-1), clkPin(-1), dcPin(-1), csPin(-1), rstPin(rst_pin)
				 {
}
/*!
...
Adafruit_SSD1306::Adafruit_SSD1306(uint8_t w, uint8_t h, int8_t mosi_pin,
		...
		Adafruit_GFX(w, h), spi(NULL), wire(NULL), buffer(NULL), mosiPin(
				mosi_pin), clkPin(sclk_pin), dcPin(dc_pin), csPin(cs_pin), rstPin(
				rst_pin) {
}
...
Adafruit_SSD1306::Adafruit_SSD1306(uint8_t w, uint8_t h, SPIClass *spi,
		...
		Adafruit_GFX(w, h), spi(spi ? spi : &SPI), wire(NULL), buffer(NULL), mosiPin(
				-1), clkPin(-1), dcPin(dc_pin), csPin(cs_pin), rstPin(rst_pin) {
...
Adafruit_SSD1306::Adafruit_SSD1306(int8_t mosi_pin, int8_t sclk_pin,
		...
		Adafruit_GFX(SSD1306_LCDWIDTH, SSD1306_LCDHEIGHT), spi(NULL), wire(
				NULL), buffer(NULL), mosiPin(mosi_pin), clkPin(sclk_pin), dcPin(
				dc_pin), csPin(cs_pin), rstPin(rst_pin) {
}
...
Adafruit_SSD1306::Adafruit_SSD1306(int8_t dc_pin, int8_t rst_pin, int8_t cs_pin) :
		Adafruit_GFX(SSD1306_LCDWIDTH, SSD1306_LCDHEIGHT), spi(&SPI), wire(
				NULL), buffer(NULL), mosiPin(-1), clkPin(-1), dcPin(dc_pin), csPin(
				cs_pin), rstPin(rst_pin) {
#ifdef SPI_HAS_TRANSACTION
...
Adafruit_SSD1306::Adafruit_SSD1306(int8_t rst_pin) :
		Adafruit_GFX(SSD1306_LCDWIDTH, SSD1306_LCDHEIGHT), spi(NULL), wire(
				&Wire), buffer(NULL), mosiPin(-1), clkPin(-1), dcPin(-1), csPin(
				-1), rstPin(rst_pin) {
}
	...
	if ((x >= 0) && (x < WIDTH)) { // X coord in bounds?
		...
		if (__h > 0) { // Proceed only if height is now positive
			...
			if (mod) {
				...
				static const uint8_t PROGMEM premask[8] = { 0x00, 0x80, 0xC0,
			...
			if (h >= mod) { // More to go?
				...
				if (h) { // Do the final partial byte, if necessary
					...
					// note - lookup table results in a nearly 10% performance
					// improvement in fill* functions
					static const uint8_t PROGMEM postmask[8] = { 0x00, 0x01,
...
void Adafruit_SSD1306::startscrolldiagright(uint8_t start, uint8_t stop) {
	...
	ssd1306_command1(0X00);
	ssd1306_command1(stop);
	static const uint8_t PROGMEM scrollList3c[] = { 0X01,
	SSD1306_ACTIVATE_SCROLL };
...
void Adafruit_SSD1306::startscrolldiagleft(uint8_t start, uint8_t stop) {
	...
	ssd1306_command1(0X00);
	ssd1306_command1(stop);
	static const uint8_t PROGMEM scrollList4c[] = { 0X01,
	SSD1306_ACTIVATE_SCROLL };
