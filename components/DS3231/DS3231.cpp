...
const uint8_t daysArray[] PROGMEM = { 31, 28, 31, 30, 31, 30, 31, 31, 30, 31,
		30, 31 };
...
void DS3231::setDateTime(uint16_t year, uint8_t month, uint8_t day,
...
void DS3231::setAlarm1(uint8_t dydw, uint8_t hour, uint8_t minute,
		uint8_t second, DS3231_alarm1_t mode, bool armed) {
...
void DS3231::setAlarm2(uint8_t dydw, uint8_t hour, uint8_t minute,
		DS3231_alarm2_t mode, bool armed) {
	minute = dec2bcd(minute);
	hour = dec2bcd(hour);
	dydw = dec2bcd(dydw);
...
long DS3231::time2long(uint16_t days, uint8_t hours, uint8_t minutes,
		uint8_t seconds) {
...
uint32_t DS3231::unixtime(void) {
	...
	u = time2long(date2days(t.year, t.month, t.day), t.hour, t.minute,
			t.second);
	u += 946681200;
...
uint8_t DS3231::dow(uint16_t y, uint8_t m, uint8_t d) {
	...
	y -= m < 3;
	dow = ((y + y / 4 - y / 100 + y / 400 + pgm_read_byte(dowArray + (m - 1))
			+ d) % 7);
