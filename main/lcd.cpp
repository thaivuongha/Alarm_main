...
void show_time(void) {
	...
	while (1) {
		//This code displays the Time only hours:minutes
...
void testdrawcircle(void) {
	...
	for (int16_t i = 0; i < max(display.width(), display.height()) / 2; i +=
			2) {
...
void testfillcircle(void) {
	...
	for (int16_t i = max(display.width(), display.height()) / 2; i > 0; i -=
			3) {
...
void testfillroundrect(void) {
	...
	for (int16_t i = 0; i < display.height() / 2 - 2; i += 2) {
		...
		display.fillRoundRect(i, i, display.width() - 2 * i,
				display.height() - 2 * i, display.height() / 4, INVERSE);
		display.display();
		delay(1);
	}
